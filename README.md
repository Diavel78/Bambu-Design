...
| `box_DEEP.stl`  | 3.94″ × 7.875″ | **2×** | tall front bins (small drawer) |
| `box_BRUSH.stl` | 2.625″ × 11.375″| **1×** | full-length brush / pencil tray |

That's **13 boxes total**: 3 in the small drawer (M across the back, two DEEP
bins up front), 10 in the big drawer. Each
drawer ends up with ~⅛″ of wiggle room in both directions so the set drops in
easily but doesn't slide around.

### Printing (H2D)

- **PLA**, 0.2 mm layers, **2–3 walls**, 10–15 % infill, no supports.
- The brush tray is 289 mm long — fine on the H2D bed, print it straight.
- Walls are 1.6 mm / floors 1.2 mm, so each box is quick and light
  (~50–100 cm³ of plastic each).
- Print them in different colors from the AMS if she wants a color-coded
  drawer — each box is its own STL, so just pick a filament per plate.

Everything is generated by `src/generate_vanity_organizer.py` — tell me if you
want different compartment splits (more small bins, a divided lipstick tray,
shallower boxes, label tabs…) and I'll regenerate.
